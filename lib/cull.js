var cull = (function (global) {
    ...
    /**
     * Returns the result of applying `fn` to `initial` and the first item
     * in `seq`, then applying `fn` to that result and the 2nd item, etc.
     *
     * Can also be called without `initial`, in which case the first invocation
     * of `fn` will be with the first two items in `seq`.
     */
    ...
    function some(pred, seq) {
        var i, l;
        for (i = 0, l = seq.length; i < l; ++i) {
            if (pred(seq[i])) { return true; }
        }
        return false;
    }

    /**
     * Is `pred` truthy for at least one item in `seq`, and also falsy
     * for at least one item in `seq`?
     */
