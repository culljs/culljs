var cull = (function (global) {
    ...
    function flatten(list) {
        ...
        for (i = 0, l = list.length; i < l; i++) {
            result = result.concat(isList(list[i]) ? flatten(list[i]) : list[i]);
        }
    ...
    (function () {
        function children(elements) {
            if (isList(elements)) { return flatten(map(children, elements)); }
        ...
        function get(selectors, parent) {
            ...
            var select = function (list, selector) {
                if (/^#/.test(selector)) {
                    return list.concat(id(selector.slice(1)));
                } else if (/^\./.test(selector)) {
                    var cn = selector.slice(1);
                    return concat(list, parent.getElementsByClassName(cn));
                } else {
                    return concat(list, parent.getElementsByTagName(selector));
                }
            };
            return uniq(reduce(select, [], toList(selectors)));
        }
        ...
        function frag(items) {
            ...
            doall(bind(fragment, "appendChild"), toList(items));
            return fragment;
        }
